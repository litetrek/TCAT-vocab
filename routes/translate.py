...
from repositories import terms_repo, glossary_repo
...
# tagging) and from ai_context (the term-scoped "Ask AI" doctrinal gloss).

@translate_bp.route("/api/trans/books/<int:book_id>/glossary", methods=["GET"])
@_require_translation
def api_list_glossary(book_id):
    """Any translation-module user can view a book's glossary."""
    try:
        return jsonify(glossary_repo.list_for_book(book_id))
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500


@translate_bp.route("/api/trans/books/<int:book_id>/glossary", methods=["POST"])
@_require_translation
def api_add_glossary_term(book_id):
    if not is_leader():
        return jsonify({"error": "Leader role or higher required"}), 403
    data = request.get_json() or {}
    term_display_id = (data.get("term_id") or "").strip()
    if not term_display_id:
        return jsonify({"error": "term_id is required"}), 400
    try:
        modifier = session["user_email"]
        now_str  = datetime.now().strftime("%Y-%m-%d %H:%M")
        gid = glossary_repo.add_term(book_id, term_display_id, modifier, now_str)
        if gid is None:
            return jsonify({"error": "Term not found, or already in this book's glossary"}), 400
        entry = glossary_repo.get_response(gid)
        write_audit(term_display_id, entry.get("term_chinese", ""), modifier, session.get("user_name", ""),
                    "updated", details="Added to glossary")
        return jsonify(entry), 201
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500


@translate_bp.route("/api/trans/glossary/<int:glossary_id>/generate", methods=["POST"])
@_require_translation
def api_generate_glossary_entry(glossary_id):
    if not is_leader():
        return jsonify({"error": "Leader role or higher required"}), 403
    row = glossary_repo.get_one(glossary_id)
    if not row:
        return jsonify({"error": "Glossary entry not found"}), 404
    term = row.get("terms") or {}
    book = row.get("trans_books") or {}
    try:
        explanation = generate_glossary_entry(
            chinese     = term.get("chinese", ""),
            pinyin      = term.get("pinyin", ""),
            pali        = term.get("pali", ""),
            sanskrit    = term.get("sanskrit", ""),
            known_trans = term.get("translation_known", ""),
        ...
        now_str  = datetime.now().strftime("%Y-%m-%d %H:%M")
        modifier = session["user_email"]
        glossary_repo.update_entry(glossary_id, explanation=explanation, explanation_source="ai",
                                    modifier=modifier, now_str=now_str)
        write_audit(term.get("display_id", ""), term.get("chinese", ""), modifier, session.get("user_name", ""),
                    "ai_translated", details="Glossary entry (re)generated by AI")
        return jsonify(glossary_repo.get_response(glossary_id))
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500


@translate_bp.route("/api/trans/glossary/<int:glossary_id>", methods=["PATCH"])
@_require_translation
def api_update_glossary_entry(glossary_id):
    if not is_leader():
        return jsonify({"error": "Leader role or higher required"}), 403
    data = request.get_json() or {}
    row = glossary_repo.get_one(glossary_id)
    ...
    if status is not None and status not in ("draft", "reviewed"):
        return jsonify({"error": "status must be 'draft' or 'reviewed'"}), 400
    try:
        modifier = session["user_email"]
        now_str  = datetime.now().strftime("%Y-%m-%d %H:%M")
        old_explanation = row.get("explanation") or ""
        glossary_repo.update_entry(glossary_id, explanation=explanation, status=status,
                                    modifier=modifier, now_str=now_str)
        write_audit(term.get("display_id", ""), term.get("chinese", ""), modifier, session.get("user_name", ""),
                    "updated", field_changed="Glossary entry",
                    old_value=old_explanation, new_value=explanation if explanation is not None else old_explanation)
        return jsonify(glossary_repo.get_response(glossary_id))
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500


@translate_bp.route("/api/trans/glossary/<int:glossary_id>", methods=["DELETE"])
@_require_translation
def api_delete_glossary_entry(glossary_id):
    if not is_leader():
        return jsonify({"error": "Leader role or higher required"}), 403
    row = glossary_repo.get_one(glossary_id)
    if not row:
    ...
    term = row.get("terms") or {}
    try:
        removed = glossary_repo.remove_term(row["book_id"], glossary_id)
        if not removed:
            return jsonify({"error": "Glossary entry not found"}), 404
        write_audit(term.get("display_id", ""), term.get("chinese", ""), session["user_email"], session.get("user_name", ""),
                    "updated", details="Removed from glossary")
